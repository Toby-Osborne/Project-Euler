#include <vector>
#include <iostream>
#include <numeric>
#include <bits/stdc++.h>
#include <cmath>
#include<algorithm>
#include <sstream>
using namespace std;


int main(){
    //Clock Begin
    clock_t begin = clock();
    double time_spent = 0.0;
    vector <int> vec;
    long long int bigindex = 0;
    long long int bignum = 0;
    
    string longnumber = "";
    long long int num;

    string v;
    for (unsigned int i = 0;i<longnumber.size();i++){
        v = longnumber[i];
        vec.push_back(stoi(v));
        //cout << vec.back();
    }
    //cout << endl;
    

    //Clock

    for (unsigned int i = 0;i<(vec.size()-13);i++) {
        num = 1;
        for (int j = 0;j<13;j++) {
            num = num*((long long int)vec[i+j]);
            //cout << vec[i+j];
        }
        //cout << endl;
        //cout << "product = " << num<< endl;
        //cout << num << endl;
        if (num > bignum) {
            bigindex = i;
            bignum = num;
        }
        //cout << num << endl;
    }
    for (int i = 0;i<13;i++) {
        cout << vec[bigindex+i];
    }
    cout << endl;
    cout << "product: " << bignum << endl;

    

    clock_t end = clock();
    time_spent += (double)(end - begin) / CLOCKS_PER_SEC;
    cout << "Time Spent: " << time_spent << endl;
    return 0; 
}